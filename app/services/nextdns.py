...
async def create_profile(api_key_or_keys, log_callback=None):
    if isinstance(api_key_or_keys, str):
        keys = [k.strip() for k in api_key_or_keys.split(",") if k.strip()]
    elif isinstance(api_key_or_keys, list):
        keys = api_key_or_keys
    else:
        keys = []

    for key in keys:
        pid, link = await _create_single_profile(key, log_callback)
        if pid and link:
            return pid, link
            
    return None, None

async def _create_single_profile(api_key, log_callback=None):
    def log(msg):
        if log_callback:
            log_callback(msg)
